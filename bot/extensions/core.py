...
class Core(discord_commands.Cog):
    ...
    @discord_commands.command(brief=core_config['clear_brief'],
    ...
    async def info(self, ctx):
        embed_msg = DiscordEmbed(title=bot_config['name'], color=self.embeds_color)
        embed_msg.add_field(name='Version', value=f"`{bot_config['version']}`")
        embed_msg.add_field(name='Source', value=f"[GitHub repository]({bot_config['url']})")
        embed_msg.add_field(name='\u200b', value='\u200b')
        embed_msg.add_field(name='License', value=bot_config['license'])
        embed_msg.add_field(name='Author', value=bot_config['author'])
        embed_msg.add_field(name='\u200b', value='\u200b')
    ...
    async def whoami(self, ctx):
        embed_msg = DiscordEmbed(description=f"You are {ctx.message.author.mention}", color=self.embeds_color)
        await ctx.send(embed=embed_msg)
